...
function updateStats(rowsData) {
  let grandTotals = Array(shareholders.length).fill(0);
  rowsData.forEach(row => {
    row.values.forEach((v, i) => grandTotals[i] += v);
  });
  const top3 = grandTotals.map((total, i) => ({ name: shareholders[i], total })).sort((a, b) => b.total - a.total).slice(0, 3);
  const topList = document.getElementById("topShareholders");
  topList.innerHTML = '<ul>' + top3.map((item, idx) => `<li>${idx + 1}. ${item.name}: ${item.total.toLocaleString()} USD</li>`).join('') + '</ul>';
}
...
// ====== Export/Import CSV (Tính Năng Mới) ======
function exportCSV() {
  const data = JSON.parse(localStorage.getItem("shareData"));
